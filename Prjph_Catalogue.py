        ...
        try:
            for k,v in dict_exif.items():
                ...
                if type(v)==PIL.TiffImagePlugin.IFDRational:
                    dict_exif[k]='IFDRational '+str(v)   #original
                    #findebug
                ...
                if k == 42034: #éventuellement à remplacer par un test sur type tuple s'il y en a d'autres
                    dict_exif[k]=str(v)
                if k == 532: #une 5-uplet de valeurs de type IDFRational => mise en string (suite bug3 sur un fichier)
                    dict_exif[k]=str(v)
                #je retire les données bytes de plus de 12 de long
                if type(v) == bytes and len(str(v)) > 12:
                    listpop.append(k)

            #Suppression des données à supprimer
...
testRepertoire = r"C:\Users\LENOVO\Documents\Projets\Prj_photos\Prjph_repertoire_test\Test_bug3\cefichier"

f='Numériser0011 (2).jpg'
#f='JL_091103 019.jpg'
#f='test.jpg'
#f='Captureph1.jpg'
#f='Capturefondbl.JPG'
    ...
    for f in mylistfile: #ATTENTION : LES ELEMENTS DE mylistfile SONT DES NT.ENTRIES (pas des strings)
        ...
        if f_name.find('.')!=-1: #il y a un '.' (donc une extension), le fichier est traité. Sinon on passe au suivant.
            ...
            if f_ext in fileExtList:    #ajout de .lower() pour être insensible à la casse
                ...
                duree_restante, hfin_estimated = fnph_duree_restante(start_time, datetime.now(), nb_totalfiles_treated, 
                                                            monRepertoire_files_imgouvid) 

                msgtodisplay1="   <lus/inRep:"+ str(cpt) + "/" + str(lenlistfile) + ") " +                               "[" + percentage + "] " +                               "... Mdata: " + f_name + " ---" +                               " (cumul:"+ str(nb_totalfiles_treated) +"/"+ str(monRepertoire_files_imgouvid) + ")"+                               "timeleft:" + str(duree_restante).split('.')[0] + "/" + str(hfin_estimated) + ">"
                              #Remplacée : " (cumul:"+ str(nb_totalfiles_treated) +"/"+ str(monRepertoire_totalnb_files) + ")"+ \
                ...
                if not fnph_alreadyexists_in_db(parm_directory_name, f.name):
                    ...
                    if "st_ctime" in fileDict["data"]["stats"]:
                        if fileDict["data"]["stats"]["st_ctime"] < 0:
                            ...
                            fileDict["data"]["stats"]["st_ctime_comment"]    = "forcé à 0 - ancienne valeur : " +                                                                             str(var_st_ct_time)
                            fileDict["data"]["stats"]["st_ctime_ns_comment"] = "forcé à 0 - ancienne valeur : " +                                                                             str(var_st_ct_time_ns)
    ...
    for i in range(nb_files_in_rep):
        ...
        if mylist[i]["already_exist_in_db"]:
           ...
           #fieldname = 'events.event_'+str(strtimestamp)
            ...
            #INSERT
            #print('\ndebug-->0<fnph_cat_f...> -> insertion de mylist[',i,'] --------', str(mylist[i]),'\n')
...
def fnph_getstats_dir(parm_dir):
    ...
    Ajout : elle renvoie aussi le nombre de fichier de type image ou vidéo
    """
    import tqdm
    
    #création de la liste des types (extensions) de fichier (image et vidéo)
    my_list_types=[k.lower() for k,v in mes_types_images.items()]+[k.lower() for k,v in mes_types_videos.items()]
    cpt_files_images_ou_videos=0

    nb_files   = 0
    nb_folders = 0

    for _, dirnames, filenames in tqdm.tqdm(os.walk(parm_dir)): # _ for 'root' not used here
        nb_files   += len(filenames)
        nb_folders += len(dirnames)
        #et calcul du nombre de fichiers de type image ou vidéo pour cumul
        for f in filenames:
            try:
                cpt_files_images_ou_videos += '.'+(f.split('.')[1]).lower() in my_list_types
            except:
                continue

    #print("{:,} files, {:,} folders".format(files, folders))
    return [nb_files, nb_folders, cpt_files_images_ou_videos]
...
path=r"D:\Svg_PCMSI_P_180307\Utilisateur"
#path=r"C:\Users\LENOVO\Documents\Projets\Prj_photos\Prjph_repertoire_test\Screenshots"
...
mylist=['Capture d’écran (1).png', 'Capture d’écran (2).png', 'Capture d’écran (3).png', 'Capture d’écran (4).png', 'Capture d’écran (5).png', 'desktop.ini']
'.png' in str(mylist)
...
cpt_files_images_ou_videos=0
for f in mylist:
    print(f, '.'+(f.split('.')[1]).lower() in my_list_types)
    cpt_files_images_ou_videos += '.'+(f.split('.')[1]).lower() in my_list_types

cpt_files_images_ou_videos
